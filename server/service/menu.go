...
func hasParentMenu(menuID string, c *db.Client) (model.Menu, bool, error) {
	res, exists, err := getParentMenuWithQuery(c, menuID, getParentMenusQuery)
	if err != nil {
		return model.Menu{}, false, err
	}

	if exists {
		return res.Parents[0], exists, nil
	}
	return model.Menu{}, exists, nil
}

func hasChildMenu(menuID string, c *db.Client) ([]model.Menu, bool, error) {
...
func isParentMenuChanged(current, oldParent model.Menu) bool {
